...
const RAW_COUNTS = [89, 142, 98, 215, 187, 54, 42];

const MOCK_WEEKLY = Array.from({ length: 7 }, (_, i) => {
  const d = new Date();
  d.setDate(d.getDate() - (6 - i));
  return {
    day: i === 6 ? '오늘' : DAY_NAMES[d.getDay()],
    count: RAW_COUNTS[i],
  };
});
...
const RATIO_TOTAL = MOCK_RATIO.reduce((s, d) => s + d.value, 0);
...
const CustomTooltipPie = ({ active, payload }: any) => {
  ...
  return (
    <div className="tooltip-popup" style={TOOLTIP_STYLE}>
      ...
      <div style={{ color: '#555' }}>
        {n.toLocaleString()}건 ({Math.round((n / RATIO_TOTAL) * 100)}%)
      </div>
    </div>
  );
};
      ...
      <div className="chart-grid">
        ...
        <div className="card chart-card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={MOCK_WEEKLY} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
              <XAxis
                dataKey="day"
        ...
        <div className="card chart-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <PieChart width={260} height={220}>
            ...
            >
              {MOCK_RATIO.map((_, i) => (
                <Cell key={i} fill={RATIO_COLORS[i]} strokeWidth={0} />
              ))}
            </Pie>
            <Tooltip
